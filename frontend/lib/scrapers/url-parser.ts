...
} from './accommodation-scraper';

export class UrlParserService {
  private scraper: AccommodationScraper;

  constructor() {
    this.scraper = new AccommodationScraper();
  }

  async parseAccommodationUrl(url: string): Promise<ParsedAccommodationData> {
    try {
      // First try Node.js scraper for detailed extraction
      const scrapedData = await this.scraper.scrapeAccommodation(url);

      // If scraper returned valid data, use it
